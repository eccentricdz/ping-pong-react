...
const Scoreboard = ({ playerWon, playerName, score, livesLeft, robotLevel, topScore, gameInProgress }) => {
    const [scoreBoardState, updateScoreBoardState] = useState("active");
    const [callToAction, updateCallToAction] = useState("Click anywhere to begin!");
    ...
    return (
        <div className={`scoreBoard ${scoreBoardState}`}>
            <div className="column-1 column">
                <Gamestat name="robotLevel" header="Level" value={robotLevel} highlight={robotAttitude == "angry" ? true : false}></Gamestat>
                <Gamestat name="livesLeft" header="Lives left" value={livesLeft} kind="hearts" highlight={robotAttitude == "happy" ? true : false}></Gamestat>
            </div>
            ...
            <div className="column-3 column">
                <Gamestat name="highScore" header="Top score" value={topScore}></Gamestat>
